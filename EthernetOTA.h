#ifndef __ETHERNET_OTA_H
#define __ETHERNET_OTA_H
...
class EthernetOTAClass
{
  public:
    ...
    EthernetOTAClass& setPort(uint16_t port);
    ...
    EthernetOTAClass& setPasswordHash(const char *password);

    //Sets the partition label to write to when updating SPIFFS. Default NULL
    EthernetOTAClass &setPartitionLabel(const char *partition_label);
    String getPartitionLabel();

    //Sets if the device should be rebooted after successful update. Default true
    EthernetOTAClass& setRebootOnSuccess(bool reboot);

    //Sets if the device should advertise itself to Arduino IDE. Default true
    EthernetOTAClass& setMdnsEnabled(bool enabled);

    //This callback will be called when OTA connection has begun
    EthernetOTAClass& onStart(THandlerFunction fn);

    //This callback will be called when OTA has finished
    EthernetOTAClass& onEnd(THandlerFunction fn);

    //This callback will be called when OTA encountered Error
    EthernetOTAClass& onError(THandlerFunction_Error fn);

    //This callback will be called when OTA is receiving data
    EthernetOTAClass& onProgress(THandlerFunction_Progress fn);

    //Starts the EthernetOTA service
    bool begin();

    //Ends the EthernetOTA service
    void end();
